...
def manual_open(files,ds_slice={'x':slice(700,2100), 'y':slice(700,1200)},variables=["CMI_C04","CMI_C05","CMI_C06","CMI_C07","CMI_C08","CMI_C09","CMI_C10","CMI_C11","CMI_C12","CMI_C13","CMI_C14","CMI_C15","CMI_C16","goes_imager_projection"]):
    for k in range(len(files)):
        ...
        if k == 0:
            ...
            DS = xr.combine_nested([DS, DS2], concat_dim=["t"], combine_attrs = "override")
            store.close()
    return DS
def main_function(year,day0,hour0,hourf):
    ...
    for j in formated_hours[hour0-1:hourf-1]:
        ...
        files = [file_root + str(year) + "/" + formated_days[day0-1] + "/" + j + "/" + file for file in glob.glob("*")]
    ...
    for i in new_DCC:
        ...
        first_coords = [int(np.round(np.mean(np.where(growth_markers==i),1)[1:])[0]),int(np.round(np.mean(np.where(growth_markers==i),1)[1:])[1])]
        init_lat = lat[first_coords[0],first_coords[1]]
        init_lon = lon[first_coords[0],first_coords[1]]        
        
        mask_column = not_dcc[:,first_coords[0]-search_radius:first_coords[0]+search_radius+1,first_coords[1]-search_radius:first_coords[1]+search_radius+1]
        ...
        for j in sample_locs:
            ...
            search_range = 4
            d=4
            clipped = goes_ds.isel({'t':slice(j-search_range,j),'y':slice(first_coords[0]-d,first_coords[0]+d+1),'x':slice(first_coords[1]-d,first_coords[1]+d+1)})
            if (clipped.x.size == d*2+1) and (clipped.y.size == d*2+1) and (clipped.t.size == search_range):
                if globe.is_land(init_lat, init_lon):
                    #this is on land
                    file_name = '/gws/nopw/j04/aopp/jowanf/land/negdata/DCC_'+str(year)+'_'+str(pd.DatetimeIndex(clipped.t).dayofyear[0])+'_'+str(hour0)+'_'+str(hourf)+'_'+str(i)+'_'+str(j)
                    clipped.to_netcdf(file_name)
                else:
                    #this is over the sea
                    file_name = '/gws/nopw/j04/aopp/jowanf/sea/negdata/DCC_'+str(year)+'_'+str(pd.DatetimeIndex(clipped.t).dayofyear[0])+'_'+str(hour0)+'_'+str(hourf)+'_'+str(i)+'_'+str(j)
                    clipped.to_netcdf(file_name)
        ...
        d=4
        clipped = goes_ds.isel({'t':slice(first_appearence-search_range,first_appearence),'y':slice(first_coords[0]-d,first_coords[0]+d+1),'x':slice(first_coords[1]-d,first_coords[1]+d+1)})
        #save the positive examples
        if (clipped.x.size == d*2+1) and (clipped.y.size == d*2+1) and (clipped.t.size == search_range):
            print("okay to save!")
            if globe.is_land(init_lat, init_lon):
                file_name = '/gws/nopw/j04/aopp/jowanf/land/data/DCC_'+str(year)+'_'+str(pd.DatetimeIndex(clipped.t).dayofyear[0])+'_'+str(hour0)+'_'+str(hourf)+'_'+str(i)
                clipped.to_netcdf(file_name)
            else:
                file_name = '/gws/nopw/j04/aopp/jowanf/sea/data/DCC_'+str(year)+'_'+str(pd.DatetimeIndex(clipped.t).dayofyear[0])+'_'+str(hour0)+'_'+str(hourf)+'_'+str(i)
                clipped.to_netcdf(file_name)
