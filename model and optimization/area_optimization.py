...
def main_function(num_trimmed,c_num,domain):
    ...
    X_zoomed = np.reshape(np.reshape(X,(len(Y),8,4,9,9))[:,:,:,num_trimmed:9-num_trimmed,num_trimmed:9-num_trimmed],(len(Y),8*4*int((9-2*num_trimmed)**2)))
    ...
    file_write = '/home/users/jowanf/area_optimize_'+domain+'_'+str(num_trimmed)+'.txt'
